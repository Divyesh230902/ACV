    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 200) {
        ...
        if (data.length !== 0) {
          ...
          var firstResult = data.fruit_output; // Changed the name to firstResult
          var fruit_image = data.img;
          // Display the plate number in the result div
          resultDivText.innerHTML = "The fruit is " + firstResult;
          // Display the plate image in the result img
          console.log(fruit_image)
          document.getElementById ('image-id').src = fruit_image;
